...
#méthode qui définit la configuration streaming client
def client_stream
...
#selectionner 5 journalistes de manière aléatoire et créer un nouveau tableau
login = 5.times.map { journalist [rand(0...journalist.length)]}.uniq
end 
#envoyer un message aux journalistes séléctionnés
def tweet (client1,journalists_name) 
 
login.each do |i|
   puts client_connect ('Hey #{i} ! I like your last post #bonjour_monde @the_hacking_pro') 
end 

tweet (client_connect, login) 
#retrouver les dernières personnes qui ont twitté
def like(client1)
    client_connect.search ( "#bonjour_monde", result_type : "recent").take (25).each do |tweet|
    puts client_connect.favorite(tweets) 
    end 
end 
like (client
#suivre ces personnes
def follow(client1) 
    client_connect.search ( "#bonjour_monde", result_type : "recent").take (20).each do |tweet|
    puts client_connect.follow(tweet.user)
    end 
end 
...
def live(client2) 
    ...
    puts client_stream.fav object
    puts client_stream.follow (object.user) 
    end 
end 
